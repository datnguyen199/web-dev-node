...
exports.connectDB = function() {
    ...
    .then(yamltext => {
        return yamltext.safeLoad(yamltext, 'utf8');
    })
    .then(params => {
...
exports.destroy = function(username) {
    ...
    .then(user => {
        if(!user) throw new Error('Did not find record ' + username + ' to delete');
        user.destroy;
        return;
    })
}
...
exports.userPasswordCheck = function(username, password) {
    ...
    .then(user => {
        if(!user) {
            return { checked: false, username: username, message: 'Could not find user' };
        } else if(user.username === username && user.password === password) {
            return { checked: true, username: username }
        } else {
            return { checked: false, username: username, message: 'Incorrect username or password' }
        }
    })
}
